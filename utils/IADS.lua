...
locla SAMRangeLookupTable = { -- Ranges at which SAM sites are considered close enough to activate in m
    ["Kub 1S91 str"] = 52000,
 ...
 if site.Enabled then
    ...
    if site.TrackFiles then
      for _, track in pairs(site.TrackFiles) do
        if track.Position and utils.getDistance(site.Location, track.Position) < (site.EngageRange * 1.15) then
          inRange = true
        end
      end
    end
...
local function enableSAM(site)
  ...
  local hasAmmo = ammoCheck(site)
    if utils.tablelength(site.ControlledBy) > 0 then
...
local function associateSAMS()
  for _, EWR in pairs(EWRSites) do
    ...
    for _, SAM in pairs(SAMSites) do
      if SAM.SAMGroup:getCoalition() == EWR.EWRGroup:getCoalition() and utils.getDistance3D(SAM.Location, EWR.Location) < EWRAssociationRange then
  ...
  for _, EWR in pairs(EWRSites) do
    ...
    for j, targets in pairs(detections) do
      if targets.object and targets.object:isExist() and targets.object:inAir() then
        ...
        IADS.TrackFiles["EWR"][trackName] = EWR.trackFiles[trackName]
  ...
  for _, SAM in pairs(SAMSites) do
    ...
    for _, targets in pairs(detections) do
      if targets.object and targets.object:isExist() and targets.object:inAir() then
        ...
        IADS.TrackFiles["SAM"][trackName] = SAM.trackFiles[trackName]
  ...
  for _, AWACS in pairs(AWACSAircraft) do
    ...
    for _, targets in pairs(detections) do
      if targets.object and targets.object:isExist() and targets.object:inAir() then
        ...
        IADS.TrackFiles["AWACS"][trackName] = AWACS.trackFiles[trackName]
      end
    end
  end
  return timer.getTime() + 2
...
local function EWRSAMOnRequest()
  for _, SAM in pairs(SAMSites) do
    if(utils.tablelength(SAM.ControlledBy) > 0) then
      local viableTarget = false
      for _, EWR in pairs(SAM.ControlledBy) do
        for _, target in pairs(EWR.trackFiles) do
          if target.Position and utils.getDistance(SAM.Location, target.Position) < SAM.EngageRange then
            viableTarget = true
          end
        end
      end
...
local function BlinkSAM()
  for _, SAM in pairs(SAMSites) do
    if utils.tablelength(SAM.ControlledBy) < 1 then
  ...
  if IADSRadioDetection then
    if event.weapon then
      ...
      if WeaponDesc.guidance == 5 then
        for _, SAM in pairs(SAMSites) do
          if math.random(1,100) < IADSARMHidePctage and utils.getDistance(SAM.Location, WeaponPoint) < IADSARMHideRangeRadio then
            magnumHide(SAM)
          end
        end
      end
  ...
  for _, EWR in pairs(EWRSites) do
    for _, track in pairs(EWR.trackFiles) do
      if ((timer.getAbsTime() - track.LastDetected) > trackMemory or (not track.Object:isExist()) or (not track.Object:inAir())) then
        ...
        IADS.TrackFiles.EWR[track.Name] = nil
      end
    end
  end
  for _, SAM in pairs(SAMSites) do
    for _, track in pairs(SAM.trackFiles) do
      if ((timer.getAbsTime() - track.LastDetected) > trackMemory or (not track.Object:isExist()) or (not track.Object:inAir())) then
        ...
        IADS.TrackFiles.SAM[track.Name] = nil
      end
    end
  end
  for _, AWACS in pairs(AWACSAircraft) do
    for _, track in pairs(AWACS.trackFiles) do
      ...
      AWACS.trackFiles[track.Name] = nil
        IADS.TrackFiles.AWACS[track.Name] = nil
      end
    end
  end
  return timer.getTime() + 2
