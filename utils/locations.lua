...
function locations.getNearestTown(map, pnt)
  ...
  local pntNear
  local pntLL = {}
  pntLL.latitude, pntLL.longitude, pntLL.altitude = coord.LOtoLL(pnt)
  for _, twn in pairs(locations[map].towns) do
    dist = utils.getDistanceLL({latitude = twn.latitude, longitude = twn.longitude},
      {latitude = pntLL.latitude, longitude = pntLL.longitude} )
    if nearest == nil then
      nearest = twn.display_name
      distNear  = dist
      pntNear = {longitude = twn.longitude, latitude = twn.latitude}
  ...
  ret = {loc = nearest, dis =  distNear, twnPt = pntNear, brng = utils.bearingToPt(pntLL,pntNear)}
  return ret
end
